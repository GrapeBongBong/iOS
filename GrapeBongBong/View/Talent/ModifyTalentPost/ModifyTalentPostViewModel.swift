//
//  ModifyTalentPostViewModel.swift
//  GrapeBongBong
//
...
class ModifyTalentPostViewModel: ObservableObject {
    ...
    func sendRequest(with post: Post, token: String) {
        guard let json = try? JSONEncoder().encode(post) else { return }
        
        let url = URL(string: URLList.baseURL + "/exchange/post/\(post.id)")!
